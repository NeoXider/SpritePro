...
- **Привязка к сцене**: Таймер обновляется только когда сцена активна
...
- `scene` (Scene | str, optional): Сцена, в которой таймер активен. По умолчанию: None
...
# Таймер, активный только в конкретной сцене
scene_timer = s.Timer(duration=2.0, callback=on_tick, scene="hud")
...
- [Здоровье](health_component.md) — регенерация здоровья
- [Анимация](../core/animation.md) — тайминг кадров
- [Tween](../core/tween_system.md) — плавные переходы
