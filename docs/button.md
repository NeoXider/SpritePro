...
- `scene` (Scene | str): Сцена для кнопки и её текста. По умолчанию: None

По умолчанию кнопка в режиме **screen_space=True**: позиция и размер не зависят от камеры и зума. Для кнопки в мировых координатах вызовите `.set_screen_space(False)`.
