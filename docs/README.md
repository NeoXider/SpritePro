# Документация SpritePro

Добро пожаловать в документацию SpritePro! Здесь вы найдете **подробную** информацию о всех модулях и компонентах библиотеки.

## Быстрый обзор

**[Обзор возможностей (OVERVIEW)](OVERVIEW.md)** — краткий саммари: Layout, мультиплеер, физика, Builder, основные подсистемы и демо.

**Полный указатель** — [DOCUMENTATION_INDEX.md](../DOCUMENTATION_INDEX.md): все документы по категориям, демо-игры, рекомендуемый порядок изучения и последние обновления.

---

## Обзор модулей

SpritePro состоит из нескольких основных модулей, каждый из которых предоставляет специфическую функциональность для разработки игр.

### Основные компоненты

#### [Sprite](sprite.md)
Базовый класс спрайта с основной функциональностью: позиционирование, отрисовка, обновление и управление компонентами.

### UI компоненты

#### [Button](button.md)
Интерактивная кнопка с поддержкой различных состояний (обычное, наведение, нажатие) и обработкой событий.

#### [ToggleButton](toggle_button.md)
Переключатель, который переключается между состояниями ВКЛ/ВЫКЛ с разными цветами и текстовыми метками.

#### [Text](text.md)
Компонент для отрисовки и управления текстом с поддержкой различных шрифтов, размеров и цветов.

#### [Mouse Interactor](mouse_interactor.md)
Компонент для обработки взаимодействия с мышью: обнаружение наведения, клики и другие события мыши.

#### [DraggableSprite](draggable_sprite.md)
Drag-and-drop компонент на базе Sprite с колбэками и возвратом на место.

#### [Pages](pages.md)
Система страниц (Page/PageManager) для UI и экранов внутри сцены.

#### [Layout](layout.md)
Автолейаут для дочерних спрайтов (наследует Sprite): flex, сетка, окружность, линия; контейнер — спрайт, rect или сам лейаут (container=None).

### Игровые системы

#### [Animation](animation.md)
Система анимации с поддержкой состояний, кадров, параллельных анимаций и интеграции с tweening.

#### [Tween](tween.md)
Система для плавных переходов и анимаций с различными типами easing и управлением временем. **Fluent API (Do-твины)**: `sprite.DoMove(...).SetEase(Ease.OutQuad).OnComplete(...)` — автоудаление по завершении, циклы (reset/yoyo), `Kill(complete=...)`.

#### [Tween Presets](tween_presets.md)
Готовые твины для позиции, масштаба, поворота, цвета, прозрачности и размера.

#### [Timer](timer.md)
Система таймеров для точного управления временем в играх: обратный отсчет, задержки, повторяющиеся события.

#### [Physics](physics.md)
Система физики (pymunk): типы тел (DYNAMIC, STATIC, KINEMATIC), PhysicsWorld, PhysicsConfig, PhysicsBody, PhysicsShape (формы коллайдеров: AUTO, BOX, CIRCLE, LINE), колбэк on_collision, границы мира. Поворот с физикой не синхронизируется (обновляется только позиция). Подробное описание API и демо (physics_demo, hoop_bounce_demo, ping_pong).

#### [Builder](builder.md)
Fluent API для спрайтов и частиц: SpriteBuilder (position, scale, color, crop, border_radius, mask, build), ParticleBuilder. Полная таблица методов и примеры.

#### [Networking](networking.md)
Минимальные TCP-хелперы для простого мультиплеера.

#### [Particles](particles.md)
Настраиваемые всплески частиц с временем жизни (секунды или мс), гравитацией, порядком сортировки, частицами на основе изображений, вращением и масштабированием.

#### [Health](health.md)
Система управления здоровьем: урон, лечение, регенерация, максимальное здоровье и обработка смерти.

#### [Игровой цикл](game_loop.md)
Базовый цикл и сцены для разделения экранов (меню, игра, пауза) с переключением и перезапуском.

#### [Ввод и события](input.md)
InputState в стиле Unity и EventBus для подписки на события.

#### [Debug Overlay](debug.md)
Сетка мира, координаты камеры и отладочные логи.

### Утилиты

#### [Surface](surface.md)
Набор утилит для работы с поверхностями Pygame: скругленные углы, маскирование и другие эффекты.

#### [Color Effects](color_effects.md)
Динамические цветовые эффекты и анимации: пульсация, радуга, дыхание, волна и другие визуальные эффекты.

### Готовые спрайты

#### [Обзор готовых спрайтов](readySprites.md)
Полное руководство по предустановленным игровым компонентам с немедленной готовностью к использованию и полной настройкой.

#### [Text_fps](text_fps.md)
Готовый спрайт-счетчик FPS, который автоматически отображает и обновляет частоту кадров с настраиваемым внешним видом.

### Готовые сцены (readyScenes)

Модуль **spritePro.readyScenes** содержит сцены, которые можно подключать в своих играх без копирования кода.

- **ChatScene** — мультиплеерный чат: история сообщений, поле имени, ввод текста, скролл (колёсико и перетаскивание мышью), маска по области скролла, время в сообщениях (ЧЧ:ММ:СС). Требует инициализированный мультиплеер (`s.multiplayer.init_context(net, role)`). Подключение: `from spritePro.readyScenes import ChatScene, ChatStyle`; затем `s.scene.add_scene("chat", ChatScene)` и `s.scene.set_scene_by_name("chat", recreate=True)`. Стиль задаётся классом **ChatStyle** (цвета, шрифты, отступы).

## Быстрая навигация

### По типу функциональности

**Базовые компоненты:**
- [Sprite](sprite.md) - Основа всех игровых объектов
- [Builder](builder.md) - Fluent API: спрайты и частицы (crop, border_radius, mask)
- [Physics](physics.md) - Физика тел, коллизии, on_collision

**Пользовательский интерфейс:**
- [Button](button.md) - Интерактивные кнопки
- [ToggleButton](toggle_button.md) - Переключатели
- [Text](text.md) - Текстовые элементы
- [Mouse Interactor](mouse_interactor.md) - Обработка мыши
- [DraggableSprite](draggable_sprite.md) - Drag-and-drop на базе Sprite

**Анимация и эффекты:**
- [Animation](animation.md) - Анимация на основе кадров
- [Tween](tween.md) - Плавные переходы

**Игровая логика:**
- [Timer](timer.md) - Управление временем
- [Networking](networking.md) - Минимальный мультиплеер (TCP)
- [Health](health.md) - Система здоровья
- [Debug Overlay](debug.md) - Отладочная сетка и логи
- [Pages](pages.md) - Страницы и менеджер страниц
- [Layout](layout.md) - Автолейаут (flex, сетка, круг, линия)

**Вспомогательные инструменты:**
- [Surface](surface.md) - Операции с поверхностями
- [Color Effects](color_effects.md) - Динамические цветовые эффекты и утилиты
- [Игровой цикл](game_loop.md) - Базовый цикл и сцены
- [Ввод и события](input.md) - InputState и EventBus

**Готовые спрайты и сцены:**
- [Обзор готовых спрайтов](readySprites.md) - Руководство по предустановленным игровым компонентам
- [Text_fps](text_fps.md) - Автоматический отображение счетчика FPS
- Готовые сцены (readyScenes): ChatScene — мультиплеерный чат (см. раздел выше)

## Примеры использования

Каждый модуль содержит подробные примеры использования и объяснения API. Рекомендуется начать с [Sprite](sprite.md) как основы, затем изучить специализированные компоненты в зависимости от ваших потребностей.

Для быстрого старта можно использовать генератор шаблона проекта:
```bash
python create_project.py MyGame
```

## Интеграция модулей

Модули SpritePro разработаны для совместной работы. Например:
- `Sprite` может использовать компоненты `Health`, `MouseInteractor` и `Timer`
- `Animation` может интегрироваться с `Tween` для создания сложных эффектов
- Компоненты могут использоваться вместе для создания сложных игровых систем

Изучите документацию каждого модуля, чтобы понять возможности интеграции.
