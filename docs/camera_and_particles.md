...
- `s.shake_camera(strength=(12, 12), duration=0.35)` — дрожание камеры с перезапуском.
...
**Важно:** Параметр `dt` автоматически передается зарегистрированным объектам из `spritePro.dt`, поэтому не нужно передавать его вручную.
