...
- **С лобби** (`use_lobby=True`): одно окно. Сначала показывается экран настройки: имя, выбор «Хост» / «Клиент», порт (и для клиента — IP). После подключения — список игроков (roster), для хоста кнопка «В игру». По нажатию «В игру» вызывается ваша `multiplayer_main(net, role)`. Для готового приложения достаточно заменить вызов на `s.networking.run(use_lobby=True)`.
...
1. **Один экземпляр (хост)**  
   ...
   - Появится экран «В лобби»: список игроков (сначала только Host), кнопка **«В игру»**.
...
2. **Второй экземпляр (клиент)**  
   ...
   - Нажать **«Подключиться»**.  
   - Появится экран «В лобби»: список игроков (Host, Player 1), кнопка **«Готов»** (переключатель: Готов / Готов ✓).  

3. **Старт игры**  
   - Хост нажимает **«В игру»**.  
   - У **обоих** игроков закрывается лобби и запускается игра (`multiplayer_main(net, role)`).  
   - Клиент может нажимать **«Готов»** для смены состояния; хост при желании может учитывать событие `ready` в своей логике.

### Для разработчика

- **События лобби:** хост рассылает `start_game` при нажатии «В игру»; клиент при получении `start_game` тоже переходит в игру. Клиент отправляет `ready` с `{"value": True/False}` при нажатии «Готов».
...
- **use_lobby=True** (в коде) — одно окно с лобби: настройка (имя, хост/клиент, порт, IP), подключение, roster, кнопка «В игру». Не используется при запуске через `--quick` / env (тогда роль уже задана).
