...
- `handle_keyboard_input(up_key=K_UP, down_key=K_DOWN, left_key=K_LEFT, right_key=K_RIGHT)`: Обработать ввод с клавиатуры для движения с настраиваемыми клавишами

### Визуальные эффекты
- `set_scale(scale)`: Масштабировать спрайт равномерно
- `get_scale()`: Получить текущий коэффициент масштабирования
- `set_alpha(alpha)`: Установить прозрачность (0-255)
- `get_alpha()`: Получить текущее значение альфа-канала
- `rotate_to(angle)`: Повернуть спрайт на конкретный угол в градусах
- `get_angle()`: Получить текущий угол поворота
- `rotate_by(angle_change)`: Повернуть спрайт на относительный угол
- `set_color(color)`: Применить цветовую тонировку (передайте `None` для удаления)
- `get_color()`: Получить текущую цветовую тонировку
- `fade_by(amount, min_alpha=0, max_alpha=255)`: Изменить прозрачность на относительное значение с ограничениями
- `scale_by(amount, min_scale=0.0, max_scale=2.0)`: Изменить масштаб на относительное значение с ограничениями
- `set_flip(flip_h, flip_v)`: Установить отражение по горизонтали и вертикали
- `set_image(image_source, size=None)`: Установить новое изображение для спрайта (путь к файлу или Surface)
- `set_native_size()`: Сбросить спрайт к оригинальным размерам изображения
...
### Позиция и размер
- `set_position(position, anchor=Anchor.CENTER)`: Установить позицию спрайта с якорем
- `get_position()`: Получить текущую центральную позицию (x, y)
- `get_size()`: Получить текущий размер (ширина, высота)
- `get_world_position()`: Получить мировую позицию спрайта (Vector2), учитывая иерархию
...
Все визуальные и связанные с движением свойства теперь имеют соответствующие методы получения:

- `get_color()`: Возвращает текущую цветовую тонировку
- `get_scale()`: Возвращает текущий коэффициент масштабирования
- `get_alpha()`: Возвращает текущее значение альфа-канала (0-255)
- `get_angle()`: Возвращает текущий угол поворота в градусах
- `get_position()`: Возвращает текущую центральную позицию (x, y)
- `get_size()`: Возвращает текущий размер (ширина, высота)
- `get_velocity()`: Возвращает текущую скорость (vx, vy)
...
sprite.play_sound("sound.wav")
```

**Примечание:** Звук загружается и воспроизводится при каждом вызове. Для оптимизации кэшируйте объекты `pygame.mixer.Sound`
...
# Применить визуальные эффекты через методы
player.set_scale(1.5)
player.set_alpha(200)
player.rotate_to(45)

# Или через свойства (более удобно и читаемо)
player.scale = 1.5
player.alpha = 200
player.angle = 45
