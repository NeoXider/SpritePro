...
class FPSCameraDemo:
    def __init__(self):
        ...
        # Create FPS display
        self.fps_text = s.TextSprite("FPS: 0", 24, (255, 255, 0), (10, 10))
        self.camera_text = s.TextSprite("Camera: (0, 0)", 20, (200, 200, 255), (10, 40))
        self.instructions_text = s.TextSprite("Use ARROW KEYS to move camera", 18, (150, 150, 150), (10, 70))
    ...
    def create_world_objects(self):
        ...
        labels = [
            "Start Area", "Forest", "Mountain", "Lake", "Village", 
            "Castle", "Desert", "Ocean", "Cave", "Temple"
        ]
    ...
    def update_fps(self):
        ...
        if s.dt > 0:
            ...
            self.camera_text.set_text(f"Camera: ({self.camera.x:.0f}, {self.camera.y:.0f})")
    ...
    def draw_world_objects(self):
        ...
        for obj in self.world_objects:
            if obj['type'] == 'square':
                ...
                screen_rect = pygame.Rect(screen_pos[0], screen_pos[1], obj['size'][0], obj['size'][1])
                
                # Only draw if visible on screen (simple culling)
                if (screen_rect.right >= 0 and screen_rect.left <= 1000 and 
                    screen_rect.bottom >= 0 and screen_rect.top <= 700):
                ...
                if (screen_pos[0] + obj['radius'] >= 0 and screen_pos[0] - obj['radius'] <= 1000 and
                    screen_pos[1] + obj['radius'] >= 0 and screen_pos[1] - obj['radius'] <= 700):
                    pygame.draw.circle(self.screen, obj['color'], 
                                     (int(screen_pos[0]), int(screen_pos[1])), obj['radius'])
                    pygame.draw.circle(self.screen, (255, 255, 255), 
                                     (int(screen_pos[0]), int(screen_pos[1])), obj['radius'], 2)
    ...
    def draw_text_sprites(self):
        ...
        for obj in self.object_sprites:
            ...
            # Only draw if visible on screen
            if (screen_pos[0] >= -200 and screen_pos[0] <= 1200 and
                screen_pos[1] >= -50 and screen_pos[1] <= 750):
    ...
    def draw_grid(self):
        ...
        for x in range(start_x, start_x + 1200, grid_size):
            ...
            if 0 <= screen_x <= 1000:
                pygame.draw.line(self.screen, (40, 40, 40), (screen_x, 0), (screen_x, 700))
        
        # Horizontal lines
        for y in range(start_y, start_y + 800, grid_size):
            screen_y = y - self.camera.y
            if 0 <= screen_y <= 700:
                pygame.draw.line(self.screen, (40, 40, 40), (0, screen_y), (1000, screen_y))
    
    def draw_ui(self):
        """Draw UI elements (not affected by camera)."""
        # Draw semi-transparent background for UI
        ui_bg = pygame.Surface((300, 100))
        ui_bg.set_alpha(128)
        ui_bg.fill((0, 0, 0))
        ...
        perf_text = s.TextSprite(f"Objects: {len(self.world_objects)}", 16, (150, 255, 150), (10, 100))
        perf_text.update(self.screen)
        
        frame_text = s.TextSprite(f"Frame: {self.frame_count}", 16, (255, 150, 150), (10, 120))
