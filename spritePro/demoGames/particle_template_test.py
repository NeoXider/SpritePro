...
class FountainParticle(Particle):
    ...
    def check_collision_with_falling(
        ...
        if (
            not self.has_collided_with_falling
            and isinstance(falling_particle, FallingParticle)
            and not falling_particle.has_collided
            and self.rect.colliderect(falling_particle.rect)
        ):
...
class FallingParticle(Particle):
    ...
    def check_ground_collision(self, ground_y):
        ...
        if not self.has_hit_ground and self.rect.bottom >= ground_y:
            ...
            self.scale = 1.5
            s.debug_log_info(f"Falling particle hit ground at y={ground_y}")

    def check_particle_collision(
        self, other_particle, collision_messages_list, collision_count_ref
    ):
        """Проверка столкновения с другой частицей."""
        if (
            not self.has_collided
            and isinstance(other_particle, FallingParticle)
            and not other_particle.has_collided
            and self.rect.colliderect(other_particle.rect)
            and self != other_particle
        ):
            # Столкновение обнаружено
    ...
    for i, particle1 in enumerate(falling_particles_list):
        ...
        for particle2 in falling_particles_list[i + 1 :]:
            if particle2.has_collided:  # Пропускаем уже столкнувшиеся
                continue
            if particle1.check_particle_collision(
                particle2, collision_messages, collision_count_ref
            ):
    ...
    for fountain_particle in fountain_particles_list:
        ...
        for falling_particle in falling_particles_list:
            if falling_particle.has_collided:  # Пропускаем уже столкнувшиеся
                continue
            if fountain_particle.check_collision_with_falling(
                falling_particle, collision_messages, collision_count_ref
            ):
    ...
    for i, msg_data in enumerate(collision_messages):
        ...
        if msg_timer <= 0:
            ...
            s.screen.blit(
                shadow_surface,
                (
                    int(msg_pos.x - shadow_surface.get_width() // 2) + 2,
            ...
            s.screen.blit(
                text_surface,
                (
                    int(msg_pos.x - text_surface.get_width() // 2),
        ...
        if isinstance(particle, FallingParticle) and particle.active:
            ...
            if particle.impact_effect_timer > 0:
                ...
                effect_surface = pygame.Surface(
                    (effect_radius * 2, effect_radius * 2), pygame.SRCALPHA
                )
                pygame.draw.circle(
                    effect_surface,
                    (255, 255, 0, effect_alpha),
                ...
                s.screen.blit(
                    effect_surface,
                    (
                        particle.rect.centerx - effect_radius,
                        particle.rect.centery - effect_radius,
                    ),
                )
            ...
            if particle.collision_effect_timer > 0:
                ...
                pygame.draw.circle(
                    effect_surface,
                    (255, 150, 50, effect_alpha),
                    (effect_radius, effect_radius),
                    int(effect_radius * 0.7),
                )
                # Внутренний белый круг
                pygame.draw.circle(
                    effect_surface,
                    (255, 255, 255, effect_alpha),
                    (effect_radius, effect_radius),
                    effect_radius // 2,
                )
                s.screen.blit(
