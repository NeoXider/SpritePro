...
def spawn_scene(
    ...
    *,
    ...
    """Спавнит сцену редактора в игру: s.spawn_scene("level.json").

    Ленивая обёртка над spritePro.editor.runtime.spawn_scene (без циклического импорта).

    Args:
        scene_path (str | Path): Путь к JSON-файлу сцены редактора.
        scene (Scene, optional): Игровая сцена для спавна. По умолчанию текущая.
        apply_camera (bool, optional): Применить камеру из сцены. По умолчанию True.
        enable_static_cache (bool, optional): Включить static cache. По умолчанию False.

    Returns:
        RuntimeScene: Заспавненная runtime-сцена.
    """
    from .editor.runtime import spawn_scene as _spawn_scene
...
__all__ = [
    ...
    "set_debug_perf_enabled",
    "set_debug_perf_style",
    "get_perf_snapshot",
...
def _sync_globals() -> None:
    ...
    global WH, WH_C, VISIBLE_RECT, VISIBLE_WH, VISIBLE_WH_C
    global SAFE_RECT, SAFE_WH, SAFE_WH_C
    global screen, screen_rect, dt, pygame_events, clock, frame_count, FPS
    global time_since_start
    WH = _context.WH
    WH_C = _context.WH_C
    VISIBLE_RECT = _context.visible_rect.copy()
    VISIBLE_WH = Vector2(_context.visible_rect.size)
    VISIBLE_WH_C = Vector2(_context.visible_rect.center)
    SAFE_RECT = _context.safe_rect.copy()
    SAFE_WH = Vector2(_context.safe_rect.size)
    SAFE_WH_C = Vector2(_context.safe_rect.center)
...
def set_debug_perf_enabled(enabled: bool = True) -> None:
    """Включает или выключает встроенный профайлер стадий кадра."""
    _context.game.set_debug_perf_enabled(enabled)
...
def set_debug_perf_style(
    *,
    ...
    _context.game.set_debug_perf_style(
...
def get_perf_snapshot():
    ...
    return _context.game.get_perf_snapshot()
...
def _extract_platform_args(
    argv: list[str],
    default_platform: str,
) -> tuple[str, list[str]]:
    platform = default_platform.strip().lower() or "pygame"
    filtered: list[str] = []
    for arg in argv:
        if arg == "--kivy":
            platform = "kivy"
...
def get_screen(
    ...
    Args:
        ...
        resizable (bool, optional): Разрешить ли пользователю менять размер desktop-окна.
...
_RUN_MULTIPLAYER_ENTRY = "__spritepro_run_multiplayer_entry__"


def _call_with_supported_args(func: Callable[..., object], *args: object) -> object:
    try:
    ...
    call_count = min(len(positional_parameters), len(args))
    required_count = sum(
        1 for parameter in positional_parameters if parameter.default is inspect.Parameter.empty
    )
    if required_count > call_count:
        raise TypeError(
            f"{getattr(func, '__name__', repr(func))} requires {required_count} positional args, "
            f"but only {call_count} are supported by SpritePro."
        )
    return func(*args[:call_count])


def _resolve_scene_value(scene, *scene_args: object):
    if scene is None:
        return None
    if isinstance(scene, Scene):
        return scene
    if isinstance(scene, type) and issubclass(scene, Scene):
        return _call_with_supported_args(scene, *scene_args)
    if callable(scene):
        resolved = _call_with_supported_args(scene, *scene_args)
...
def _emit_resize_event(force: bool = True) -> None:
    _sync_globals()
    _context.dispatch_resize_event(force=force)
...
def run(
    ...
    *,
    ...
    multiplayer_argv: list[str] | None = None,
    multiplayer_entry: Callable[..., object] | str | None = None,
    multiplayer_host: str = "127.0.0.1",
    multiplayer_port: int = 5050,
    multiplayer_clients: int = 2,
    multiplayer_server_tick_rate: int = 30,
    multiplayer_net_debug: bool = False,
    multiplayer_client_spawn_delay: float = 0.0,
    multiplayer_use_lobby: bool = False,
) -> object | None:
    """Запускает SpritePro-игру в pygame или Kivy без ручного цикла.

    В браузере (pygbag/wasm, sys.platform == "emscripten") возвращает awaitable
    (Task или корутину) вместо блокирующего цикла — рекомендуется `await s.run(...)`;
    без await игра всё равно запустится, если event loop уже работает.
    Multiplayer в браузере не поддерживается (нет TCP-сокетов) и даёт RuntimeError.
    ...
    Args:
        ...
        reference_size: Виртуальное разрешение рендера. Например `(1920, 1080)`,
            ...
            для full HD.
        resizable: Разрешить ли пользователю менять размер desktop-окна в pygame-режиме.
        title: Заголовок окна.
        icon: Иконка desktop-окна.
        fps: Целевой FPS.
        fill_color: Цвет очистки кадра.
        platform: "pygame" или "kivy".
        multiplayer: Если True, `s.run(...)` становится точкой входа и для сетевого режима.
            Тогда можно использовать те же terminal-аргументы `--quick`, `--server`,
            `--host_mode`, `--host`, `--port`, `--clients`.
        multiplayer_argv: Явный список argv для multiplayer runner. Если None,
            берётся `sys.argv[1:]`.
        multiplayer_entry: Своя entry-функция для `s.networking.run(...)`.
            Если не задана, SpritePro автоматически создаёт внутреннюю entry-функцию
            и передаёт в `scene`/`setup` объекты `net` и `role`, если их сигнатура это поддерживает.
        multiplayer_host: IP сервера.
        multiplayer_port: Порт сервера.
        multiplayer_clients: Общее число окон для `--quick` (host + clients).
        multiplayer_server_tick_rate: Тикрейт сервера в режиме `--server`.
        multiplayer_net_debug: Включить сетевой debug.
        multiplayer_client_spawn_delay: Задержка запуска клиентов в `--quick`.
        multiplayer_use_lobby: Запуск через готовое лобби вместо quick/server/client flow.
    """
    argv = list(multiplayer_argv) if multiplayer_argv is not None else list(sys.argv[1:])
    platform, filtered_multiplayer_argv = _extract_platform_args(argv, platform)
    size, title, platform = _resolve_run_options(size, title, platform)

    if multiplayer and sys.platform == "emscripten":
        raise RuntimeError(
            "s.run(multiplayer=True) не работает в браузере: WebAssembly не поддерживает "
            "TCP-сокеты. Соберите desktop-версию или уберите multiplayer для web-сборки."
        )

    if multiplayer:
        from . import networking as _networking

        if multiplayer_entry is None:

            def _generated_multiplayer_entry(net, role) -> None:
                def _wrapped_setup():
                    if setup is None:
                        return None
                    return _call_with_supported_args(setup, net, role)
            ...
            entry_callable: Callable[..., object] = _generated_multiplayer_entry
            entry_name = _RUN_MULTIPLAYER_ENTRY
        elif isinstance(multiplayer_entry, str):
            entry_callable = None  # type: ignore[assignment]
            entry_name = multiplayer_entry
        else:
            entry_callable = multiplayer_entry
            entry_name = _RUN_MULTIPLAYER_ENTRY
        ...
        if entry_callable is not None:
            ...
            if main_module is None:
                raise RuntimeError(
                    "Не удалось определить __main__ для регистрации multiplayer entry."
                )
        ...
        _networking.run(
            ...
            clients=multiplayer_clients,
            server_tick_rate=multiplayer_server_tick_rate,
            net_debug=multiplayer_net_debug,
            client_spawn_delay=multiplayer_client_spawn_delay,
            use_lobby=multiplayer_use_lobby,
        )
        return
    ...
    if platform_normalized in {"pygame", "desktop"}:
        ...
        if sys.platform == "emscripten":
            # Web (pygbag/wasm): блокирующий while True заморозил бы вкладку браузера —
            # цикл обязан кооперативно уступать через await asyncio.sleep(0) каждый кадр.
            # Возвращаем awaitable; если event loop уже запущен (стандартный случай под
            # pygbag), игра дополнительно планируется как task — заработает даже если
            # вызывающий код забыл сделать `await s.run(...)`.
            import asyncio
        ...
        kivy_error: str | None = None
        try:
            from .mobile import run_kivy_app
            from kivy.core.window import Window as _kivy_window

            if _kivy_window is None:
                kivy_error = "kivy Window недоступен (headless-окружение или нет дисплея)"
        except Exception as exc:
            kivy_error = str(exc)

        if kivy_error is not None:
            logging.getLogger("spritePro").warning(
                "platform='kivy' недоступна (%s) — запускаю на pygame", kivy_error
            )
            return run(
                setup,
                scene=scene,
