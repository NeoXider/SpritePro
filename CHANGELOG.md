...
### Changed
- 🔤 **Кнопка в редакторе — как в Unity: Button + дочерний Text** — надпись кнопки теперь живёт в дочернем Text-объекте иерархии (редактируется как обычный текст, можно сдвигать). Нет ребёнка-текста — кнопка без надписи (икон-кнопки). В рантайме дочерний Text становится `btn.text_sprite` и не спавнится отдельным объектом. Legacy-сцены с текстом в `custom_data` кнопки работают как раньше.
...
### Added
- 🧰 **Редактор: панель инструментов как в Unity** — Select/Move/Rotate/Scale постоянно видны в углу viewport (иконки, подсветка активного, тултипы с горячими клавишами); меню Tools убрано.
- ➕ **Редактор: быстрое создание объектов** — кнопка «+» в панели иерархии и правый клик по пустому месту viewport открывают меню Create (Image/Rect/Circle/Ellipse/Text/Button); объект создаётся в точке клика/центре вида.
- 📦 **`s.spawn_scene(...)`** — топ-левел алиас загрузки сцен редактора.
- 🔎 **Резолв пути сцены как у картинок** — `spawn_scene("main_level")` находит `scenes/main_level.json` рядом со скриптом, в scenes/ и assets/; расширение необязательно; при неудаче — FileNotFoundError со списком проверенных путей.
- 🧭 **Эргономика RuntimeScene** — `rt["name"]` (KeyError с подсказкой похожих имён), `in`, `len`, итерация, `rt.names()`, `rt.on_click("btn", fn)`.
- 🧹 **Шаблоны CLI** — `--create` очищен от лишней структуры (6 файлов вместо 12); новый `--create --simple` — один файл с одной сценой.
- 🎓 **Мультиплеерный курс обновлён** — актуализирован под 3.12 (ping, `get_net_stats`, TCP_NODELAY), новая секция «Диагностика сети» в уроке 10, рабочий `example_decorators.py` в уроке 11 (битый пример поправлен), испытания в уроках 1/2/3/5/6/7 (эхо-шифровальщик, лагометр, дуэль реакции, поздняя синхронизация, перетягивание зоны, матч до 3 побед), починены битые ссылки, удалён дубликат-черновик m11.

### Fixed
- 🐛 **Unity-семантика активности иерархии** — выключенный ребёнок больше не включается при цикле выключения/включения родителя: у спрайта появились `active_self` (своё состояние) и `active` (эффективное, с учётом родителей); `set_parent` пересчитывает активность.
- 🐛 **Урок 11 курса** — пример использовал `send_every` без обязательного `interval`.

### Changed
- 🔧 **property_actions редактора** — ~230 строк дублирования adjust/set сведены к единой `_apply_property` (поведение сохранено).
...
### Fixed
- 🐛 **Переносимость сцен редактора** — сцена, сохранённая с абсолютными путями к спрайтам на другой машине, показывала белые заглушки: `resolve_sprite_path` не пробовал basename рядом с файлом сцены и в assets для абсолютных путей. Найдено визуальной проверкой демо.

### Added
- 🛠️ `tools/visual_check.py` — запуск демо с реальным окном и скриншотом для визуальной проверки.
...
### Added
- 🖱️ **Кнопки в редакторе сцен** — новый тип объекта Button (GameObject → New Button, Ctrl+Shift+B): текст, размер шрифта, цвета текста/фона, размер в пикселях. `spawn_scene` создаёт настоящий `spritePro.Button`; в коде: `rt.Button("play_btn", on_click=...)`.
- 🌳 **Иерархия объектов в редакторе (как в Unity)** — drag&drop в панели иерархии делает объект ребёнком (перетаскивание на пустое место — освобождает); перемещение родителя двигает потомков; поле `parent` в JSON-формате сцены (обратная совместимость со старыми сценами); в рантайме `spawn_scene` применяет `set_parent(keep_world_position=True)`.
- 🔎 **Два пайплайна доступа к объектам сцены** — `rt.get("name")` / `rt.TextSprite("name")` возвращают объект с настройками из редактора без переопределения; `rt.TextSprite("name", text=...)` / `rt.Button(...)` / `rt.Sprite(...)` переопределяют ТОЛЬКО явно переданные параметры.
- 📡 **Пинг и сетевая статистика** — `multiplayer_ctx.ping_ms` (сглаженный RTT), `last_ping_ms`, `is_connected`, `get_net_stats()` (счётчики сообщений/байт, число клиентов); служебные `_ping`/`_pong` не попадают в `poll()` и обработчики декораторов; частота настраивается через `init_context(..., ping_interval=...)`.
- ⚡ **TCP_NODELAY** — на всех клиентских и серверных сокетах: мелкие игровые сообщения без задержки Nagle.
- 🧪 **Тестовый пакет `tests/`** — 51 pytest-тест (headless SDL): регрессии на исправленные баги ядра, компонентов, физики, частиц, мультиплеера и editor-runtime.
- 🧯 **Kivy-фолбэк** — `s.run(platform="kivy")` при недоступном kivy/дисплее переключается на pygame с предупреждением вместо падения.
- 🧹 **`ParticleEmitter.destroy()`**, **`Animation.kill()`**, **`Timer.kill()`** — снятие с глобального обновления (устранение утечек).

### Fixed
Полный аудит кодовой базы — см. `AUDIT.md` (85 проблем: 13 high / 40 med / 32 low, исправлено 80). Ключевое:
- 🐛 **Субпиксельное движение** — спрайты и частицы со скоростью < 1 px/кадр «замерзали» из-за int-усечения; теперь дробный остаток накапливается.
- 🐛 **Физика** — тело с `enabled=False` не могло включиться обратно и продолжало симулироваться; пересборка тела при изменении scale обнуляла скорость.
- 🐛 **Утечки** — `Sprite.kill()` не останавливал твины; завершённые твины не удалялись из `TweenManager`; эмиттеры/таймеры/анимации навсегда оставались в реестре обновления.
- 🐛 **Мультиплеер** — хост обрабатывал каждое сообщение дважды; коллизия id (0 зарезервирован за хостом); конкурентные `sendall` ломали фрейминг (per-socket lock); `connect()` с ретраями; `NetServer.stop()` закрывает клиентские сокеты.
- 🐛 **Редактор** — undo/redo не помечали сцену изменённой (риск потери данных); `save_scene` мутировал объекты до записи; Ctrl+C/V создавали немедленные дубли (теперь честный буфер обмена); `Timer.reset()` не работал в dt-режиме; `toggle()` кнопки не менял цвет покоя; незацикленная анимация завершалась первым кадром; сравнения `health == True`; `PoolManager` падал без инстанса; и др.
- 🐛 **asset_watcher** — новый файл вызывал перезагрузку бесконечно; полный обход диска каждый кадр (теперь раз в `poll_interval`).

### Performance
- 🚀 **Редактор** — кеширование изображений/примитивов/текста/шрифтов и трансформаций viewport: устранены тысячи дисковых stat-вызовов и пересозданий поверхностей каждый кадр.
- 🚀 **Частицы** — `transform.scale` вместо `smoothscale` при зуме; копирование атрибутов шаблона через `vars()` вместо `dir()`; убраны лишние копии Surface при эмиссии.
- 🚀 **Ядро** — O(1) регистрация update-объектов; слайдер/сетка перерисовываются только при изменении; кеш неудачных загрузок текстур; анимация масштаба кнопки с порогом прилипания.
