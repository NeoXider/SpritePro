...
### Added
- 🎓 **Урок 12 мультиплеерного курса «Архитектура сетевой игры»** — владение объектами (у каждого объекта один владелец), полный снапшот мира vs события, оптимистичное действие + подтверждение хоста, защита от гонки снапшота, подключение в середине игры (late join) без спец-логики. Рабочий пример `example_full_snapshot.py` (монеты с id, снапшоты 2 раза/с, `take_coin` → `coin_taken`), практика с TODO и решение. Перекрёстные ссылки из уроков 10–11 и README курса.
- 📖 **`ARCHITECTURE.md` в демо слизарио** — разбор host-authoritative модели: таблица владения, протокол событий, поедание через подтверждение, late join на полных снапшотах, три уровня защиты от отключений.
- ✅ **`tests/test_slisario_demo.py`** — 7 тестов сетевого протокола демо без реальной сети (FakeNet + инъекция сообщений): подтверждение еды выдаётся один раз, late join из снапшота, смерть/выход игроков, рестарт хоста сохраняет еду.
- 🛠️ `tools/smoke_timed.py` — headless smoke-запуск любого демо (SDL dummy) с жёстким тайм-аутом; удобно проверять мультиплеер без открытия окон.

### Fixed
- 🐛 **Демо слизарио: мультиплеер вообще не включался** — entry-функция без параметров не получала `net`/`role`, сцена всегда стартовала как одиночная, и весь сетевой код не выполнялся (в двух окнах шли независимые игры). Теперь `s.run(scene=GameScene, multiplayer=True)` передаёт `(net, role)` в сцену.
- 🐛 **Демо слизарио: еда «воскресала» у клиента** — клиент удалял еду локально, хост об этом не знал и возвращал её снапшотом. Теперь у еды есть id, клиент шлёт `eat_food`, растёт только по подтверждению `food_eaten`, а недавно съеденные id не восстанавливаются из снапшота.
- 🐛 **Демо слизарио: у каждого окна были свои боты и еда** — боты и еда теперь симулируются только авторитетом (хост/одиночная игра) и рассылаются `bot_state`/`food_state`; клиенты рендерят их как `NetSnakeView` без симуляции.
- 🐛 **Демо слизарио: лидерборд обрезался** — фиксированный размер `1 + BOT_COUNT` не вмещал сетевых игроков; теперь список динамический, тексты перерисовываются только при изменении.

### Changed
- 🕹️ **Демо слизарио объявлено эталоном сетевой игры SpritePro** — смерти рассылаются событием `player_died` (тело превращается в еду у хоста), отключения — `client_disconnected` → `player_left` + таймаут 5 с, рестарт хоста сохраняет еду у всех, респавн в мультиплеере в случайной точке, пинг в HUD, коллизии голова-тело по окружностям, «лоб в лоб» с ботом гибнут оба, боты гибнут о стены и змей игроков, еда не спавнится вплотную к стенам.
