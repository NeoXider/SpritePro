...
### Испытание: цвет игрока в лобби 🔥

Добавьте в меню три кнопки-квадрата (красный/зелёный/синий). Клик — ваш выбор улетает событием `"pick_color"` и применяется к вашему спрайту НА ВСЕХ экранах. Нюанс для подумать: что увидит игрок, который подключится ПОСЛЕ вашего выбора? (Подсказка: хост должен хранить выборы и досылать их новичкам — это называется «состояние поздней синхронизации».)
