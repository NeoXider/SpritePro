"""Пример 12: полный снапшот мира + авторитет хоста.

Хост владеет монетами и счётом: рассылает полный снапшот мира (world) и
подтверждает подбор монеты (coin_taken). Клиент убирает монету у себя сразу
(отзывчивость), но счёт растёт только после подтверждения хоста.

Позже подключите третье окно — оно синхронизируется само, без спец-логики:
  python multiplayer_course/12/example_full_snapshot.py --host 127.0.0.1 --port 5050
"""
...
def multiplayer_main() -> None:
    s.get_screen((800, 600), "Lesson 12 - Full Snapshot")
