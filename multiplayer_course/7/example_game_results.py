"""Пример 7: меню, игра, результаты и перезапуск.

Лобби → игра до TARGET_SCORE очков → экран результата (победитель, счёт) →
по R хост шлёт restart и все возвращаются в меню.
"""

import pygame
import spritePro as s

# Глобальный контейнер результата матча: заполняется при game_over, читается в ResultScene.
...
class GameScene(s.Scene):
    ...
    def update(self, dt):
        ...
        # Сеть: pos, score (хост считает), score_update, game_over, restart.
        for msg in ctx.poll():
            event = msg.get("event")
            data = msg.get("data", {})
            if event == "pos":
...
class ResultScene(s.Scene):
    ...
    def update(self, dt):
        ctx = _ctx()
        # Только хост по R шлёт restart; все (в т.ч. хост) переходят в меню.
...
if __name__ == "__main__":
    # Запуск примера через единый app-level API.
