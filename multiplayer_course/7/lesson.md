...
### Испытание: матч до 3 побед 🔥

Одна победа — случайность, серия — мастерство. Сделайте счёт серии: хост хранит `wins = {id: 0}`, после каждого раунда увеличивает счётчик победителя и рассылает серию вместе с `"game_over"`. Экран результатов показывает «Раунд 2/5 | 2:1». При 3 победах — финальный экран и полный сброс серии. Всё состояние серии живёт ТОЛЬКО у хоста.
