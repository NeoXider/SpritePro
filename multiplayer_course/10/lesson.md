...
> 💡 Снапшот можно раздуть до **всего мира** (все объекты со своими id) — тогда сверка с ним чинит любой рассинхрон, а новые игроки подключаются в середине игры без спец-логики. Этому посвящён отдельный урок 12.
...
## 8. Диагностика сети: ping и статистика

С версии 3.11 SpritePro сам меряет RTT (время туда-обратно) служебными пакетами `_ping`/`_pong` — они не попадают в ваш `poll()` и не мешают игре.

```python
ctx.ping_ms        # сглаженный пинг (среднее по 5 замерам), мс
ctx.last_ping_ms   # последний замер, мс
ctx.is_connected   # соединение живо?

stats = ctx.get_net_stats()
# {'ping_ms': 20.6, 'client_id': 1, 'is_host': False, 'connected': True,
#  'clients_count': 2, 'messages_sent': 21, 'messages_received': 21,
#  'bytes_sent': 1375, 'bytes_received': 1039}
```

**Сетевой HUD за 10 строк** — вешаем в угол экрана и всегда видим, что происходит:

```python
net_hud = s.TextSprite("", 16, (140, 200, 140), (10, 10), anchor=s.Anchor.TOP_LEFT)

def update_net_hud():
    st = ctx.get_net_stats()
    net_hud.set_text(
        f"ping {st['ping_ms']:.0f} ms | "
        f"out {st['messages_sent']} in {st['messages_received']} | "
        f"{st['bytes_sent'] / 1024:.1f} KB up"
    )
```

**Зачем это на практике:**
- `ping_ms` растёт → подкрутите LERP-скорость (раздел 2) под реальную задержку;
- `messages_sent` растёт как сумасшедший → вы забыли `send_every` (раздел 5);
- `bytes_sent` большой при малом числе сообщений → пакеты жирные, пора в снапшоты/дельты.

> 💡 Кстати, о задержках: SpritePro включает `TCP_NODELAY` на всех сокетах автоматически — мелкие игровые пакеты уходят сразу, без буферизации Nagle. В старых версиях это было источником «резиновых» 40–200 мс.
...
| 10 | Продвинутые темы | LERP, роутинг, оптимизация, ping/статистика |
| 11 | Сетевые декораторы | `@Command`, `@ClientRpc`, `@NetEvent` |
| 12 | Архитектура сетевой игры | Владение объектами, снапшот мира, подтверждения, late join |
